...
The writer's notes mix directives with open questions ("was anything they discussed related to X?", "is this worth including?"). A question is an assignment to investigate, not a beat to include: chase it — fetch the link, run the search — and answer it in the readback with a recommendation to include it (and where) or leave it out (and why). It joins the contract only when the writer confirms or the answer clearly earns it a place; an investigated-and-rejected angle stays out of the draft, and skipping the question is as wrong as forcing the angle in.
...
For a contest story (a primary, an election, a leadership fight): open with one framing sentence that names what the contest has become, and earn that frame with the block's own reporting. Place the first clip early — ambient scene audio from the event serves as well as an argument clip — then state the trigger event plainly. Give each principal a clip or quoted position, each side's spending or backing when sourced, and each constituency's stakes. Close on the concrete date and the question the result will answer, so the opening frame and the closing question bookend the block.

For a thin-trigger story (the writer names the day's trigger but flags that details are scarce — a closed-door meeting, a readout with no substance): do not pad the trigger and do not speculate about what happened inside it. Open on the strongest sourced development around it and build the day's context, then land the trigger mid-block ("In the midst of all of that…") carrying exactly what is knowable: the logistics, each side's own characterization, and the telling absence ("No press was in the room."). Set one sourced note of friction against a glowing official readout — a clip when you have one — and close on the point the principals agree on or the next concrete thing to watch.
